...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        if util.flipCoin(self.epsilon):
          selectedAction = random.choice(possibleActions)
        else:
          selectedAction = self.computeActionFromQValues(state)
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        cur_q = (1 - self.alpha) * prev_q + self.alpha * sample
        # updates to new q
        self.values[(state, action)] = cur_q
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward: float):
      ...
      future_q = self.computeValueFromQValues(nextState)
      # Compute the difference
      difference = (reward + self.discount * future_q) - cur_q
